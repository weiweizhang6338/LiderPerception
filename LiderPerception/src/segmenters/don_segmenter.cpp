 ...
 */
#include "../../include/segmenters/don_segmenter.hpp"
...
void DoNSegmenter::segment(const PointICloud &cloud_in,
	...
	std::cout << "Starting Difference of Normals segmentation." << std::endl;
    ...
    *cloud = cloud_in;
    ...
    if (!DoN_estimator_.initCompute()) {
		std::cout << "Could not intialize DoN feature estimator." << std::endl;
        return;
    }
    // compute DoN
	...
	std::cout << "Difference of Normals Segmentation complete. Took " << fp_ms.count() << "ms\n";
}
