 ...
 */
#include "../../include/segmenters/region_growing_segmenter.hpp"
...
RegionGrowingSegmenter::RegionGrowingSegmenter(const SegmenterParams &params)
    ...
    if (params_.rg_knn_for_normals <= 0) {
        if (params_.rg_radius_for_normals <= 0.0) {
			std::cout << "Wrong parameters for Region Growing normals estimation." << std::endl;
        } else {
            normal_estimator_omp_.setRadiusSearch(
                params_.rg_radius_for_normals);
			std::cout << "Region Growing normals estimation based on radius "
				<< params_.rg_radius_for_normals << "." << std::endl;
        }
    } else {
        normal_estimator_omp_.setKSearch(params_.rg_knn_for_normals);
		std::cout << "Region Growing normals estimation based on knn "
			<< params_.rg_knn_for_normals << "." << std::endl;
    }
...
void RegionGrowingSegmenter::segment(
    ...
    *cloud = cloud_in;
	...
	std::cout << "RegionEuclideanSegmenter Segmentation complete. Took " << fp_ms.count() << "ms\n";
	...
	fp_ms = end1 - end;
	std::cout << "Segmentation complete. Took " << fp_ms.count() << "ms\n";
}
