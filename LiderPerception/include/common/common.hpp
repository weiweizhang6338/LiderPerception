...
static bool loadPCDModel(PointCloudPtr pc,
    ...
    if (pcl::io::loadPCDFile<Point>(pcd_model_file, *pc) == -1) {
		...
		std::cout << "PCD Model " << pcd_model_file << " loaded." << std::endl;
...
static void displayPerformances(unsigned int tp,
	...
	std::cout << "Accuracy (ACC): " << static_cast<double>(tp + tn) /
		static_cast<double>(tp + fp + tn + fn) << std::endl;
	std::cout << "Sensitivity (TPR): " << true_positive_rate << std::endl;
	std::cout << "Specificity (TNR): " << true_negative_rate << std::endl;
	std::cout << "Precision: " << static_cast<double>(tp) /
		static_cast<double>(tp + fp) << std::endl;
	std::cout << "Positive likelyhood ratio: " << true_positive_rate /
